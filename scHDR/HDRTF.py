...
# 中文注释翻译 设置随机种子
# 中文注释翻译 # 中文注释翻译 seed = 1000
# 中文注释翻译 # 中文注释翻译 seed = 100
# 中文注释翻译 seed = 14
# 中文注释翻译 # 中文注释翻译 # 中文注释翻译 seed = 1
# 中文注释翻译 torch.manual_seed(seed)
# 中文注释翻译 random.seed(seed)
# 中文注释翻译 np.random.seed(seed)
...
class HeteroRGCN(nn.Module):
    ...
    def forward(self, graph, inputs):
        # 中文注释翻译 inputs are features of nodes
...
class HeteroMLPPredictor(nn.Module):
	...
	def forward(self, graph, h):
		# 中文注释翻译 h contains the node representations for each edge type computed from
		# 中文注释翻译 the GNN for heterogeneous graphs defined in the node classification
		# 中文注释翻译 section (Section 5.1).
		with graph.local_scope():
			graph.ndata['h'] = h   # 中文注释翻译 assigns 'h' of all node types in one shot
...
class HeteroDotProductPredictor(nn.Module):
	def forward(self, graph, h, etype):
		# 中文注释翻译 h contains the node representations for each node type computed from
		# 中文注释翻译 the GNN defined in the previous section (Section 5.1).
		with graph.local_scope():
			graph.ndata['h'] = h
...
def subspace_alignment(X_source, X_target, num_components):
    ...
    # 中文注释翻译 目标域可以直接用其PCA子空间进行降维，因为我们是对齐到这个子空间
...
def Edge_rename(graph, x):
    # 中文注释翻译 假设 source_neg_graph 是你的原始图
    g = graph
    # 中文注释翻译 准备一个字典，用于保存不需要被移除的边的信息
    edges_data = {}
    # 中文注释翻译 遍历原图中的所有边类型
    for canonical_etype in g.canonical_etypes:
        srctype, etype, dsttype = canonical_etype
        # 中文注释翻译 获取该类型边的源节点和目标节点ID
        src, dst = g.edges(etype=canonical_etype)
        
        if etype == 'CD':  # 中文注释翻译 如果是要移除的边类型，则更改类型名称
            new_etype = ('cell', 'CD'+x, 'drug')
        elif etype == 'DC':  # 中文注释翻译 同上
            new_etype = ('drug', 'DC'+x, 'cell')
        else:  # 中文注释翻译 如果不是要移除或更改的边类型，保持原样
...
def safe_min_max_normalize(features):
    if features.size(0) == 1:
        # 中文注释翻译 如果只有一个样本，直接返回原始特征
        return features
    
    min_val = torch.min(features, dim=0)[0]
    max_val = torch.max(features, dim=0)[0]

    # 中文注释翻译 防止除以零，设置非零最小范围
    range = max_val - min_val
    range[range == 0] = 1  # 中文注释翻译 避免除以零错误

    # 中文注释翻译 应用 Min-Max 归一化
...
def maximum_mean_discrepancy(x, y, kernel_width=1.0):
    # 中文注释翻译 Compute the Gaussian kernel matrices
